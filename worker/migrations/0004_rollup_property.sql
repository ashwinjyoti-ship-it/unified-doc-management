-- Add rollup property type (SQLite requires table rebuild to extend CHECK)
CREATE TABLE database_properties_new (
  id TEXT PRIMARY KEY,
  database_id TEXT NOT NULL REFERENCES pages(id) ON DELETE CASCADE,
  name TEXT NOT NULL,
  type TEXT NOT NULL CHECK(type IN ('text', 'number', 'date', 'select', 'multi_select', 'relation', 'rollup')),
